...
const ProductCard = memo(function ProductCard({
  ...
  onAddToCart = () => {
    console.warn(
      "onAddToCart handler is not provided to ProductCard component"
    );
  },
  onAddToWishlist = (item) => {
    console.warn(
      "onAddToWishlist handler is not provided to ProductCard component for item:",
      item
    );
  },
}) {
  ...
  useEffect(() => {
    if (product?.id) {
      const fetchRating = async () => {
        try {
          const ratingStats = await reviewUtils.getProductRatingStats(
            product.id
          );
      ...
      <div className="flex items-center mt-1 mb-2">
        <div className="flex">
          {[1, 2, 3, 4, 5].map((star) => (
            <Star
              ...
              className="mr-0.5"
              fill={
                star <= Math.round(productRating.average) ? "#F59E0B" : "none"
              }
              stroke={
                star <= Math.round(productRating.average)
                  ? "#F59E0B"
                  : "#D1D5DB"
              }
            />
        ...
        {productRating.total > 0 ? (
          <span className="ml-1 text-xs text-gray-600">
            ({productRating.average.toFixed(1)}) {productRating.total} review
            {productRating.total !== 1 ? "s" : ""}
          </span>
        ) : (
