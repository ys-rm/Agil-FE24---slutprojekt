...
export default function Navbar() {
  ...
  const [profilePic, setProfilePic] = useState("https://placehold.co/40"); // Initial to placeholder to avoid empty src
  ...
  const fetchProfileData = useCallback(async () => {
    ...
    if (cachedProfile) {
      try {
        ...
        setProfilePic(profileData.profilePic || "https://placehold.co/40");
    ...
    try {
      ...
      if (userDoc.exists()) {
        ...
        sessionStorage.setItem(
          `profile_${user.uid}`,
          JSON.stringify({
            profilePic: userData.profilePic || "https://placehold.co/40",
            name: userData.name || "User",
          })
        );

        setProfilePic(userData.profilePic || "https://placehold.co/40");
        setUserName(userData.name || "User");
      }
    } catch (error) {
      logger.error("Failed to fetch profile data", error, "Navbar");
      // Use default values on error
      setProfilePic("https://placehold.co/40");
      setUserName("User");
    } finally {
